...
describe("DeploymentDetailPage — progress and re-run/cancel", () => {
  ...
  // Attribution now comes from the authenticated session server-side (see
  // server/src/users/requireSupabaseUser.ts's req.user.name) rather than a client-supplied
  // display name -- this asserts the client no longer sends one at all.
  it("does not send a client-supplied runBy when deploying", async () => {
    vi.mocked(client.fetchDeployment).mockResolvedValue(
      baseDeployment({ status: "succeeded", components: [{ type: "ApexClass", fullName: "MyClass", action: "modify" }] })
    );
    vi.mocked(client.fetchMetadataTypes).mockResolvedValue(["ApexClass"]);
    ...
    await screen.findByText("Rerun landed");
    expect(client.rerunDeployment).toHaveBeenCalledWith("d1", expect.not.objectContaining({ runBy: expect.anything() }));
  });
...
describe("DeploymentDetailPage — scheduling", () => {
  ...
  it("schedules the draft with the chosen time, with no client-supplied runBy", async () => {
    vi.mocked(client.fetchDeployment)
      .mockResolvedValueOnce(baseDeployment({ status: "pending" }))
