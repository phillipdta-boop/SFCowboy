...
- [ ] **Step 2: Run the typecheck** (this file has no test file of its own — `pipelines/routes.test.ts`, covered in Task 10, exercises it)

Run: `cd server && npx tsc --noEmit`
Expected: errors only in files not yet converted (`pipelineRuns.ts`, `routes.ts` files, `deploy.ts`, `rollback.ts`) — confirm no NEW errors in `pipelines.ts` itself.

- [ ] **Step 3: Commit**

```bash
git add server/src/pipelines/pipelines.ts
git commit -m "feat: convert pipelines.ts to async pg"
```
